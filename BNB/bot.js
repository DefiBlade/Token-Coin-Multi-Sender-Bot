...
async function sendToken(  contract_address,
  ...
  private_key) {
    ...
    if (contract_address) {
      ...
      // Send tokens
      const txTransfer = await contract.transfer(to_address, numberOfTokens).catch(() => null);
...
const run = async () => {
  ...
  for(let i = 0 ; i < cycles ; i++) {
      ...
      let balance;
      if (config.tokenName == "BNB") {
        balance = await getBalance(provider, wallet.address);
        balance = ethers.BigNumber.from(balance) /config.decimals;
        if (balance > config.trasferFee) {
          totalBalance += (balance - config.trasferFee);
          if (config.isTransfer) {
            let balWithoutFee = (Math.floor((balance - config.trasferFee) * 1000000)/1000000).toString();
            await sendBNB(balWithoutFee, config.toAddress, wallet.address, item);
          }
          console.log(`BNB ${index++}: ${totalBalance} - ${item}`);
        }
